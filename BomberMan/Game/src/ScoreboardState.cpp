...
void ScoreboardState::render()
{
    ...
    for(auto it = mPlayers.begin(); it != mPlayers.end(); it++)
    {
        ...
        TTF_Font* font = TheGame::Instance()->getFont3();
...
bool ScoreboardState::onEnter()
{
    try
    {
        ...
        if (!stateParser.parseState("examples/GameStates.xml", mMenuID, &mGameObjects,
                                    &mTextureIDList))
    ...
    loadPlayerScores("examples/Scoreboard.txt", mPlayers);
